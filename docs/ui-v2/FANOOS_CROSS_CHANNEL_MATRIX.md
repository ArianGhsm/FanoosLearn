...
| Course | `academic_courses` / authorized academic projection | canonical course list/detail | canonical list with opaque subject-bound pagination | same | `academic.view` | Full first-class identity parity | bot deep detail may link Web; each page re-fetches canonical truth | `test_cross_channel_parity.py`; existing Web course tests |
