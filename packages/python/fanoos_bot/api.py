...
class FanoosApiClient:
    ...
    def headers(self,path: str, body: bytes) -> dict[str,str]:
        ...
        return {'Accept':'application/json','Content-Type':'application/json','X-Fanoos-Key-Id':self.key_id,'X-Fanoos-Timestamp':timestamp,'X-Fanoos-Nonce':nonce,'X-Fanoos-Content-SHA256':digest,'X-Fanoos-Signature':signature}
    def post(self,path: str,payload: dict[str,Any],*,safe_to_retry: bool=False) -> Any:
        ...
        body=self.encode(payload); attempts=3 if safe_to_retry else 1
        for attempt in range(attempts):
            res=self.sender(self.origin+path,body,self.headers(path,body),self.timeout)
            ...
            retry_after=None
            try: retry_after=float(res.headers.get('Retry-After',''))
