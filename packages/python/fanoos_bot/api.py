...
class FanoosApiClient:
    ...
    def announcements(self,platform,subject,workspace_id,limit=20,cursor=None,course_id=None):return self.post('/api/internal/v1/announcements/list',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'limit':limit,'cursor':cursor,**({'course_id':course_id} if course_id else {})},safe_to_retry=True)
    def announcement_publish(self,platform,subject,workspace_id,title,body,course_id=None):return self.post('/api/internal/v1/announcements/publish',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'title':title,'body':body,**({'course_id':course_id} if course_id else {})},safe_to_retry=False)
    ...
    def claim_notification(self,platform):return self.post('/api/internal/v1/notifications/claim',{'platform':platform},safe_to_retry=False)
    ...
    def media_claim(self):return self.post('/api/internal/v1/protected-media/claim',{},safe_to_retry=False)
    ...
    def media_fail(self,job_id,lease_token,failure_code):return self.post('/api/internal/v1/protected-media/fail',{'job_id':job_id,'lease_token':lease_token,'failure_code':failure_code},safe_to_retry=False)
    ...
    def media_forensic_candidates(self,platform,subject,workspace_id,resource_id):return self.post('/api/internal/v1/protected-media/forensic/candidates',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'resource_id':resource_id},safe_to_retry=False)
    def media_forensic_source(self,platform,subject,workspace_id,job_id,max_bytes):return self.post_binary('/api/internal/v1/protected-media/forensic/source',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'job_id':job_id},max_bytes=max_bytes,safe_to_retry=False)
    def media_forensic_resources(self,platform,subject,workspace_id,limit=10,cursor=None):return self.post('/api/internal/v1/protected-media/forensic/resources',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def create_class(self,platform,subject,identity:dict):return self.post('/api/internal/v1/classes',{'platform':platform,'subject':str(subject),**identity},safe_to_retry=True)
    def directory_provinces(self,platform,limit=10,cursor=None):return self.post('/api/internal/v1/onboarding/directory/provinces',{'platform':platform,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def directory_institutions(self,platform,province_id,limit=10,cursor=None):return self.post('/api/internal/v1/onboarding/directory/institutions',{'platform':platform,'province_id':province_id,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def directory_faculties(self,platform,institution_id,limit=10,cursor=None):return self.post('/api/internal/v1/onboarding/directory/faculties',{'platform':platform,'institution_id':institution_id,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def directory_programs(self,platform,faculty_id,limit=10,cursor=None):return self.post('/api/internal/v1/onboarding/directory/programs',{'platform':platform,'faculty_id':faculty_id,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def onboarding_otp_request(self,platform,subject,phone_number):return self.post('/api/internal/v1/onboarding/otp/request',{'platform':platform,'subject':str(subject),'phone_number':phone_number},safe_to_retry=False)
    ...
    def representative_workspaces(self,platform,subject,limit=10,cursor=None):return self.post('/api/internal/v1/representatives/workspaces',{'platform':platform,'subject':str(subject),'limit':limit,'cursor':cursor},safe_to_retry=True)
    def representative_candidates(self,platform,subject,workspace_id,limit=10,cursor=None):return self.post('/api/internal/v1/representatives/candidates',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'limit':limit,'cursor':cursor},safe_to_retry=True)
    def representative_appoint(self,platform,subject,workspace_id,target_user_id):return self.post('/api/internal/v1/representatives/appoint',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'target_user_id':target_user_id},safe_to_retry=True)
    def representative_requests_list(self,platform,subject,workspace_id):return self.post('/api/internal/v1/representatives/requests/list',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id},safe_to_retry=True)
    def representative_requests_approve(self,platform,subject,workspace_id,request_id):return self.post('/api/internal/v1/representatives/requests/approve',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'request_id':request_id},safe_to_retry=False)
    def representative_requests_decline(self,platform,subject,workspace_id,request_id):return self.post('/api/internal/v1/representatives/requests/decline',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'request_id':request_id},safe_to_retry=False)
    def class_creation_requests_list(self,platform,subject,limit=10,cursor=None):return self.post('/api/internal/v1/classes/creation-requests/list',{'platform':platform,'subject':str(subject),'limit':limit,'cursor':cursor},safe_to_retry=True)
    def class_creation_requests_approve(self,platform,subject,program_id,entry_year,cohort_label,workspace_name):return self.post('/api/internal/v1/classes/creation-requests/approve',{'platform':platform,'subject':str(subject),'program_id':program_id,'entry_year':int(entry_year),'cohort_label':cohort_label,'workspace_name':workspace_name},safe_to_retry=False)
    def class_creation_requests_decline(self,platform,subject,program_id,entry_year):return self.post('/api/internal/v1/classes/creation-requests/decline',{'platform':platform,'subject':str(subject),'program_id':program_id,'entry_year':int(entry_year)},safe_to_retry=False)
    def institution_terms_institutions(self,platform,subject,limit=10,cursor=None):return self.post('/api/internal/v1/institution-terms/institutions',{'platform':platform,'subject':str(subject),'limit':limit,'cursor':cursor},safe_to_retry=True)
    def institution_terms_set(self,platform,subject,institution_id,term_key,name,starts_on,ends_on,status='planned'):return self.post('/api/internal/v1/institution-terms/set',{'platform':platform,'subject':str(subject),'institution_id':institution_id,'term_key':term_key,'name':name,'starts_on':starts_on,'ends_on':ends_on,'status':status},safe_to_retry=False)
    def academic_terms_list(self,platform,subject,workspace_id):return self.post('/api/internal/v1/academic-terms/list',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id},safe_to_retry=True)
    def academic_terms_override(self,platform,subject,workspace_id,term_key,name,starts_on,ends_on,status='planned'):return self.post('/api/internal/v1/academic-terms/override',{'platform':platform,'subject':str(subject),'workspace_id':workspace_id,'term_key':term_key,'name':name,'starts_on':starts_on,'ends_on':ends_on,'status':status},safe_to_retry=False)
    def owner_recovery_request(self,platform,subject):return self.post('/api/internal/v1/auth/owner-recovery/request',{'platform':platform,'subject':str(subject)},safe_to_retry=False)
