...
final class SchemaContractTest
{
    ...
    public function run(): int
    {
        ...
        foreach ($migrationPaths as $path) {
            ...
            $sql .= "\n" . $contents;
            // DROP CHECK is the sole exception: MySQL has no ALTER CHECK for a
            // condition change, so widening a named CHECK constraint (e.g. a
            // mode enum gaining a value) is only expressible as dropping and
            // re-adding it, and neither step touches a table, column or row.
            // Every other DROP, and TRUNCATE, stays blocked.
