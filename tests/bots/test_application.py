...
class FakeBackend:
            ...
            "workspaces": [
                {
                    ...
                    "name": "دانشکده دندان‌پزشکی",
                },
                {
                    "id": "22222222-2222-4222-8222-222222222222",
                    "name": "پردیس علوم پزشکی",
                },
            ],
    ...
    def media_derivative_issue(self, p, s, w, j):
        ...
        return {
            "job_id": j,
            "artifact_capability": "artifact-cap",
            "size": 13,
...
class AppTest(unittest.TestCase):
    ...
    def test_course_first_navigation_is_canonical_and_no_uuid_copy(self):
        self.select()
        courses = self.app.courses("1")
        self.assertIn("📚 درس‌ها", courses.screen.text)
        self.assertIn("ترمیمی ۱", courses.screen.text)
        self.assertNotIn(self.backend.COURSE, courses.screen.text)
        detail = self.app.course_detail("1", self.backend.COURSE)
        self.assertIn("درس‌ها › ترمیمی ۱", detail.screen.text)
        labels = [button.text for row in detail.screen.rows for button in row]
        self.assertIn("📅 برنامه", labels)
        self.assertIn("📚 منابع", labels)
    ...
    def test_native_reads_use_persian_fallbacks(self):
        ...
        self.assertIn("🎓 نمرات", grades.screen.text)
        ...
        self.assertTrue(resources.screen.rows[0][0].callback.startswith("resdetail:"))

    def test_resource_detail_hides_storage_and_reauthorizes_delivery(self):
        self.select()
        detail = self.app.resource_detail("1", self.backend.RESOURCE)
        self.assertIn("جزوه پریو", detail.screen.text)
        self.assertIn("نسخه جاری مجاز", detail.screen.text)
        self.assertNotIn(self.backend.RESOURCE, detail.screen.text)
    ...
    def test_normal_purchase_ux_does_not_request_product_uuid(self):
        self.select()
        result = self.app.payments("1")
        self.assertIn("فهرست محصول", result.screen.text)
        self.assertNotIn("شناسه محصول", result.screen.text)
        labels = [button.text for row in result.screen.rows for button in row]
        self.assertIn("🌐 خرید و دسترسی در فانوس", labels)
    ...
    def test_bale_fail_closed_and_receipt(self):
        ...
        self.select()
        result = app.protected_resource("1", self.backend.RESOURCE)
    ...
    def test_update_overview_permission_gate_and_confirmation(self):
        ...
        self.assertFalse(any((button.callback or "").startswith("upd_c:") for row in denied.screen.rows for button in row))
    ...
    def test_more_is_role_aware_and_bale_never_gets_management(self):
        self.select()
        owner_more = self.app.more("owner", True)
        owner_labels = [button.text for row in owner_more.screen.rows for button in row]
        self.assertIn("⚙️ مدیریت", owner_labels)
        normal_more = self.app.more("normal", True)
        normal_labels = [button.text for row in normal_more.screen.rows for button in row]
        self.assertNotIn("⚙️ مدیریت", normal_labels)
        bale = BotApplication(self.backend, self.state, "bale", ApplicationConfig("https://fanoos.test/", "prod"))
        bale_more = bale.more("owner", True)
        bale_labels = [button.text for row in bale_more.screen.rows for button in row]
        self.assertNotIn("⚙️ مدیریت", bale_labels)
    ...
    def test_update_not_advertised_and_bale_disabled(self):
        help_text = self.app.help().screen.text
        self.assertNotIn("update_server", help_text)
        self.assertNotIn("/buy", help_text)
    ...
    def test_core_screens_have_semantic_metadata(self):
        ...
        for result in (
            self.app.home("1"),
            self.app.account("1"),
            self.app.day_schedule("1", 0),
            self.app.grades("1"),
            self.app.announcements("1"),
            self.app.resources("1"),
            self.app.courses("1"),
        ):
