...
const indexPhp = fs.readFileSync(path.join(root, 'tests/fixtures/ui-v2/index.php'), 'utf8');
const authPhp = fs.readFileSync(path.join(root, 'apps/platform/src/Identity/AuthService.php'), 'utf8');

for (const legacy of ['--line', '--muted', '--ink', '--card', '--green']) assert.equal(domainCss.includes(`var(${legacy})`), false, `legacy CSS token still used: ${legacy}`);
...
for (const match of `${appCss}\n${domainCss}`.matchAll(/var\(\s*(--[a-z0-9-]+)/gi)) assert.equal(definitions.has(match[1]), true, `unresolved CSS custom property: ${match[1]}`);
assert.equal(appJs.includes('function ensureDomainAssets()'), false, 'obsolete dynamic asset loader must be removed');
assert.equal((indexPhp.match(/\/assets\/domain-ux\.css/g) || []).length, 1, 'V2 reference domain CSS must load exactly once');
assert.equal((indexPhp.match(/\/assets\/domain-ux\.js/g) || []).length, 1, 'V2 reference domain JS must load exactly once');
assert.ok(indexPhp.indexOf('/assets/domain-ux.js') < indexPhp.indexOf('/assets/app.js'), 'V2 reference domain JS must execute before app.js');
assert.match(appJs, /setAttribute\('aria-pressed',active\?'true':'false'\)/);
assert.match(appJs, /clearActiveView\(\)/);
assert.equal(indexPhp.includes('id="dashboard" aria-labelledby="greeting" tabindex="-1"'), true, 'V2 dashboard focus target missing');
assert.equal(appJs.includes("if(focus)$('#dashboard').focus()"), true, 'V2 post-login focus handoff missing');
...
for (const view of ['schedule', 'grades', 'announcements', 'academics', 'resources', 'assessments', 'forms', 'orders']) assert.equal(indexPhp.includes(`data-view="${view}"`), true, `missing V2 shell view: ${view}`);
for (const renderer of ['schedule', 'grades', 'announcements', 'academics', 'resources', 'assessments', 'forms', 'orders', 'search']) assert.equal(domainJs.includes(renderer), true, `V2 domain renderer coverage missing: ${renderer}`);
...
console.log('integration web UX V2 reference: PASS');
