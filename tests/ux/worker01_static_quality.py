#!/usr/bin/env python3
"""Static UX guard for Worker 01 owned presentation files.

Run directly from the repository root:
    python3 tests/ux/worker01_static_quality.py

This intentionally is not wired into central CI; the integration worker may decide
how/when to promote it after all four parallel branches are reconciled.
"""
...
sensitive_patterns = {
    "environment variable": r"\bFANOOS_[A-Z0-9_]+\b",
    "authorization header": r"\bAuthorization\b",
    "secret assignment": r"\b(?:secret|api[_-]?key|access[_-]?token|bot[_-]?token)\s*[=:]",
    "chat id": r"\bchat[_-]?id\s*[=:]",
}
...
print(f" - preserved IDs: {len(required_ids)}")
print(f" - preserved data-view hooks: {len(required_views)}")
print(" - local SVG icons, design tokens, accessibility and responsive guards verified")
